(function (app) {
    app.controller('promotionListController', function ($scope, $http, commonService, apiService, notificationService) {
        $scope.properties = {
            title: "Khuyến mãi",
            function: "Danh sách",
            pageSize: 5,
            ...
            _delete: function (id, name) {
                bootbox.confirm("Bạn chắc chắn muốn xóa mã giảm giá: <b>" + name + "</b> ?", function (result) {
                    if (result) {
                        ...
                        apiService.del('DelPromotion/' + id, null, function (result) {
