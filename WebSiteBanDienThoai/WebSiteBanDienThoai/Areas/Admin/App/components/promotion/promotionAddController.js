(function (app) {
    app.controller('promotionAddController', function ($scope, $http, commonService, apiService, notificationService, $state) {
        $scope.properties = {
            title: "Khuyến mãi",
            function: "Thêm mới",
            pageSize: 5,
        ...
        $scope.methods = {
            _insert:function(){
                ...
                var date1 = new Date($scope.properties.prom.StartTime);
                $scope.properties.prom.StartTime = date1.getDate() + "/" + (date1.getMonth() + 1) + "/" + date1.getFullYear();
                var date2 = new Date($scope.properties.prom.EndTime);
                $scope.properties.prom.EndTime = date2.getDate() + "/" + (date2.getMonth() + 1) + "/" + date2.getFullYear();
				...
				apiService.post( 'AddPromotion/', $scope.properties.prom, function (result) {
					notificationService.displaySuccess($scope.properties.title+$scope.properties.prom.PromotionName + ' đã được thêm mới.');
					$state.go("promotions")
				}, function (error) {
        ...
        function Init() {
            ...
            apiService.get('GeneratePromotion', null, function (result) {
                $scope.properties.prom.PromotionName =  result.data.obj;                
            }, function (error) {
